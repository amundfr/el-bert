...
class TestKnowledgeBaseWikidata(unittest.TestCase):
    """
    Here, we only test functions where KB Wikidata is different
        from KB Wikipedia. For the rest, see test_knowledge_base_wikipedia.py
    """
    def setUp(self):
    ...
    def test_init_with_cg(self):
        ...
        subject = get_knowledge_base(self.config, wiki='data', with_cg=True)
        test_find_similar_cg(subject)
        assert subject.cand_gen is True

    def test_get_w2v_entity(self):
        """Test getting w2v entity"""
        subject = get_knowledge_base(self.config, wiki='data', with_cg=False)
        entity = 'Q744536'
    ...
    def test_find_similar_no_cg(self):
        """Test finding similar entities to an entity"""
        subject = get_knowledge_base(self.config, wiki='data', with_cg=False)
        vector = subject.wikipedia2vec.get_entity_vector(
                'Scarlett Johansson'
            )

        expected_qids = ['Q34436', 'Q93187', 'Q191644', 'Q202765', 'Q42581']
...
def test_find_similar_cg(subject):
    ...
    expected_scores = \
        [1.0, 0.39698780, 0.38836148, 0.29742372, 0.29052600]
